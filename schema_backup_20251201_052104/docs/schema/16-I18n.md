...
func (r *Registry) applyTranslations(schema *schema.Schema, translations map[string]any, locale string) *schema.Schema {
    localizedSchema := schema.Clone()
    
    // Apply schema-level translations
    if schemaTranslations, ok := translations["schema"].(map[string]any); ok {
    ...
    if fieldTranslations, ok := translations["fields"].(map[string]any); ok {
        for i := range localizedSchema.Fields {
            field := &localizedSchema.Fields[i]
            if fieldTrans, exists := fieldTranslations[field.Name].(map[string]any); exists {
...
func (r *Runtime) ValidateFieldWithLocale(ctx context.Context, fieldName string, value any) []string {
    field, exists := r.schema.GetField(fieldName)
    if !exists {
        return []string{r.i18n.GetValidationMessage("field_not_found", map[string]any{
            "field": fieldName,
        })}
    }
    ...
    if field.Required && isEmpty(value) {
        message := r.i18n.GetValidationMessage("required", map[string]any{
            "field": field.Label,
        })
    ...
    if !isValidType(value, field.Type) {
        message := r.i18n.GetValidationMessage("invalid_type", map[string]any{
            "field": field.Label,
...
// Apply in runtime
message := i18n.GetMessage("validation.min_length", map[string]any{
    "field": "First Name",
    "min":   2,
})
