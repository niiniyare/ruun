...
func (s *RuntimeState) Initialize(sch *Schema, data map[string]any) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if sch == nil {
...
func (s *RuntimeState) CreateSnapshot() any {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *RuntimeState) RestoreSnapshot(snapshot any) {
	stateSnapshot, ok := snapshot.(*StateSnapshot)
	if !ok {
...
func (s *RuntimeState) GetInitialValues() map[string]any {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *RuntimeState) GetTouchedFields() []string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	var touched []string
...
func (s *RuntimeState) valuesEqual(a, b any) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
...
func (s *RuntimeState) getDirtyCountUnsafe() int {
	count := 0
	for _, dirty := range s.dirty {
		if dirty {
			count++
...
func (s *RuntimeState) getErrorCountUnsafe() int {
	count := 0
	for _, errors := range s.errors {
		count += len(errors)
	}
...
func (s *RuntimeState) hasUnsavedChangesUnsafe() bool {
	for _, dirty := range s.dirty {
		if dirty {
			return true
		}
