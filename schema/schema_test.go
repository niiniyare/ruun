...
func (s *SchemaTestSuite) TestSchemaFrontendIntegration() {
	...
	require.Equal(s.T(), "{ open: false }", s.schema.Binding.Data)
	require.True(s.T(), s.schema.Binding.HasVisibility())
}
...
func (s *SchemaTestSuite) TestBindingReactiveFeatures() {
	binding := &Binding{
		Data: "{ open: false, toggle() { this.open = !this.open } }",
		Show: "open",
		On:   map[string]string{"click": "toggle()"},
	}
	// Test binding has visibility
	s.Require().True(binding.HasVisibility())
	// Test binding without visibility
	dataOnlyBinding := &Binding{Data: "{ value: 'test' }"}
	s.Require().False(dataOnlyBinding.HasVisibility())
	// Test nil binding
	var nilBinding *Binding
	s.Require().False(nilBinding.HasVisibility())
}
